...
class ShiftRepository(IShiftRepository):
    ...
    def get_shift_by_id(self, shift_entity: ShiftEntity) -> ShiftEntity | None:
        query = """
            SELECT id as s_id, name as s_name, description as s_description, is_active as s_is_active FROM shifts WHERE id = %s
        """
    ...
    def get_shift_by_name(self, shift_entity: ShiftEntity) -> ShiftEntity | None:
        ...
        query = """
            SELECT id as s_id, name as s_name, description as s_description, is_active as s_is_active FROM shifts WHERE name = %s
        """
        with self.conn.cursor(cursor_factory=RealDictCursor) as cur:
    ...
    def create_shift(self, shift_entity: ShiftEntity) -> bool:
        query = """
            INSERT INTO shifts (name, description) VALUES (%s, %s)
        """
    ...
    def update_shift(self, shift_entity: ShiftEntity) -> bool:
        query = """
            UPDATE shifts SET name = %s, description = %s WHERE id = %s
        """
        shift_id = shift_entity.id
        shift_name = shift_entity.name
        shift_description = shift_entity.description
    ...
    def get_all_shifts(
        ...
        # count
        count_sql = f"""
        SELECT COUNT(*) FROM shifts {qb.where_sql()}
        """
        ...
        data_sql = f"""
        SELECT id as s_id, name as s_name, description as s_description, is_active as s_is_active FROM shifts {qb.where_sql()} ORDER BY s_id DESC {limit_sql}
        """
