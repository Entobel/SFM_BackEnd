...
class RoleRepository(IRoleRepository):
    ...
    def get_list_roles(
        ...
        data_sql = f"""SELECT r.id as id, r.name as name, r.description as description, r.is_active as is_active FROM role r {qb.where_sql()} ORDER BY r.id {limit_sql}"""
    ...
    def get_role_by_name(self, name: str) -> RoleEntity | None:
        query = """SELECT r.id as id, r.name as name, r.description as description, r.is_active as is_active FROM role r WHERE r.name = %s"""
    ...
    def get_role_by_id(self, id: int) -> RoleEntity:
        query = """SELECT r.id as id, r.name as name, r.description as description, r.is_active as is_active FROM role r WHERE r.id = %s"""
