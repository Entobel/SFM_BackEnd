...
class UpdateGrowingReport(IUpdateGrowingReportUC):
    ...
    def execute(self, growing_dto, new_zone_level_ids, old_zone_level_ids, zone_id):
        ...
        growing_entity = self.growing_repo.get_growing_report_by_id(growing_entity=query_entity)

        if not growing_entity:
            raise BadRequestError("ETB_khong_tim_thay_bao_cao_growing")
        
        if growing_entity.status == GrowingStatusEnum.APPROVED.value:
            growing_entity.change_shift(new_shift=query_entity.shift)
            growing_entity.change_diet(new_diet=query_entity.diet)
            growing_entity.change_production_object(new_production_object=query_entity.production_object)
            growing_entity.change_production_type(new_production_type=query_entity.production_type)
            growing_entity.change_factory(new_factory=query_entity.factory)
            growing_entity.change_number_crates(new_number_crates=query_entity.number_crates)
            growing_entity.change_substrate_moisture(new_substrate_moisture=query_entity.substrate_moisture)
            growing_entity.change_notes(new_notes=query_entity.notes)
            growing_entity.change_status(new_status=query_entity.status)
            growing_entity.change_approved_at(new_approved_at=query_entity.approved_at)
            growing_entity.change_approved_by(new_approved_by=query_entity.approved_by)
            growing_entity.change_status(new_status=query_entity.status)
